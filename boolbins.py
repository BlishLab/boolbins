...
def safe_open_csv_file(file_name, minimum_columns=1):
    ...
    with open(file_name, 'rU') as f:
        ...
        if len(reader.next()) > minimum_columns:
            ...
            yield reader
            return
    raise Exception("Error opening csv file %s, neither comma nor tab separated" % file_name)
...
def process_files(file_names, thresholds, output_file_name, limit):
    ...
    with open(output_file_name, 'w') as output:
        ...
        # Write out the null row (that is, what % of cells have every marker below the thresholds)
        null_row = ["[]"]
        for f in files:
            freq = frequencies_by_file[f]
            total_freq_for_file = sum([freq.get(i, 0) for i in all_possible_bin_combos])
            null_row.append(1 - total_freq_for_file)
        w.writerow(null_row)
...
def run(threshold_file_name, file_names, output_file_name, limit):
    thresholds = get_thresholds_from_file(threshold_file_name)
    logging.debug("Got thresholds: %s" % thresholds)
    process_files(discover_all_files(file_names), thresholds, output_file_name, limit)
